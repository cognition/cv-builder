    ...
    def apply_selections(
        ...
        *,
        ...
        Args:
            ...
            pin_label: When set, freeze the result as a pin with selections.
        ...
        current = self.composer.loads_yaml(working.content_yaml)
        base = {
            "person": deepcopy(current.get("person") or {}),
            "skills": {"technical": [], "functional": []},
            "bio": [],
            "experience": [],
            "education": list(current.get("education") or []),
        }
        document = self.composer.build_document_from_selections(
            base, selections
        )
        new_yaml = self.composer.dumps_yaml(document)
